...
public class BPlusTree<Key extends Comparable<? super Key>, Value> {
  ...
  /**
   * Looks for the given key. If it is not found, it returns null. If it is found, it returns the associated value.
   */
  public Map<Value, Integer> find(Key key) {
    Node node = root;
    Integer count = 0;
    ...
    count += (idx + 1);// also need to add number of leaf access
    Map<Value, Integer> result = new HashMap<>();// used to put search result.
    if (idx < leaf.num && leaf.keys[idx].equals(key)) {
      result.put(leaf.values[idx], count);
    } else {
      result.put(null, -1); // not found.
    }
    return result;
  }
  ...
  class LNode extends Node {
    ...
    /**
     * Returns the position where 'key' should be inserted in a leaf node that has the given keys.
     */
  ...
  class INode extends Node {
    ...
    /**
     * Returns the position where 'key' should be inserted in an inner node that has the given keys.
     */
    public int getLoc(Key key) {
    ...
    public Split insert(Key key, Value value) {
      /*
       * Early split if node is full. This is not the canonical algorithm for B+ trees, but it is simpler and it does break the definition which might result in immature
       * split, which might not be desired in database because additional split lead to tree's height increase by 1, thus the number of disk read so first search to the leaf,
       * and split from bottom up is the correct approach.
       */
